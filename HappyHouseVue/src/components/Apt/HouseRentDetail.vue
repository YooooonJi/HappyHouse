  ...
  <div>
    <div v-if="apt">
      ...
      <ul align="left">
        ...
        <li>전용면적 : {{ apt.전용면적 }}㎡</li>
        <li>
          보증금액 : {{ (apt.보증금액.replace(",", "") * 10000) | price }}원
        </li>
        <li>
          월세금액 : {{ (apt.월세금액.replace(",", "") * 10000) | price }}원
        </li>
        <li>건축년도 : {{ apt.건축년도 }}</li>
      </ul>
  ...
  methods: {
    registerInterest: function() {
      ...
      var interest = {
        ...
        interestDeposit: (this.apt.보증금액.replace(",", "") * 10000)
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ","),
        interestMonthlyRent: (this.apt.월세금액.replace(",", "") * 10000)
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ","),
